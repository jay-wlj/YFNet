...
#ifdef _DEBUG
	#pragma comment(lib, "NetUtil_d.lib")
#else
	#pragma comment(lib, "NetUtil.lib")
#endif
...
public:
	...
	{
		m_pHttpNet = NULL;
		BOOL bHttps = url.Find(_T("https://"))>=0;
		if (bHttps)
		{
			NetUtil_CreateHttps(&m_pHttpNet);
		}
		else
		{
	...
	bool Request(std::string& outBuffer)
	{
		...
		if(error == NetError_OK)				//在调用方里用string 不可在dll中直接返回string
		{
			LONGLONG len = m_pHttpNet->GetTotalSize();
			if(len > 0)
			{
				QPtr::AutoArrayPtr<char> pBody = new char[len+1];
