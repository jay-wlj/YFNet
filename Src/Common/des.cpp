		 ...
		 int		   nKeyLen/* = 8*/)
{
	...
	if ( !pKey )
	{
		pKey = (unsigned char *)USER_PASSWORD_KEY;
		nKeyLen = strlen(USER_PASSWORD_KEY);
	}

	rv = myic_DESEncrypt(pKey, nKeyLen,padDate, (int)padDateLen, pOutData, pOutDataLen);
		   ...
		   int		     nKeyLen/* = 8*/)
{
	...
	//解密
	rv = myic_DESDecrypt(pKey, nKeyLen,pInData, nInDataLen, tmpPlain, pOutDataLen);
